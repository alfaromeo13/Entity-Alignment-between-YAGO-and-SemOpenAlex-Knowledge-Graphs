...
clone. The data, manifest, and log directories below are generated by the
preprocessing jobs and are intentionally excluded from Git because they are
...
├── manifests/           # generated SemOpenAlex shard manifests, ignored by Git
├── yago/                # generated cleaned YAGO dataset, ignored by Git
├── semopenalex_clean/   # generated cleaned SemOpenAlex dataset, ignored by Git
└── README.md
```
After preprocessing has run, the `yago/` and `semopenalex_clean/` directories
contain the final datasets used by all downstream stages of the pipeline.
