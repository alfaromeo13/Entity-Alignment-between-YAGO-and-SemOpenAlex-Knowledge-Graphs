...
The GitHub repository contains the maintained scripts, validation utilities,
visualization figures, and compact result tables. Large RDF exports and Slurm
logs are generated locally/HPC-side and are intentionally excluded from Git.

```text
07_export/
├── README.md                  
├── rdf_alignments/           # generated Turtle/TriG exports, ignored by Git
...
The `rdf_alignments/` directory is generated by the export script and may be
absent in a fresh clone. The `visualizations/` directory contains lightweight
tracked reporting artifacts generated from the final project outputs. The
`scripts/` and `validation/` directories are the maintained implementation.
