#!/usr/bin/env python3
"""Rerank Stage 06 C-only candidates with taxonomy-aware compatibility."""
...
def main() -> None:
    ...
    parser.add_argument("--min-combined-score", type=float, default=0.20)
    parser.add_argument("--embedding-weight", type=float, default=0.80)
    parser.add_argument("--neighbor-weight", type=float, default=0.20)
    args = parser.parse_args()

    print("Loading graph-neighbor scores...", flush=True)
    dataframe = pd.read_csv(
        args.scores, sep="\t", dtype=str, low_memory=False
    )
    profiles = pd.read_csv(
        args.yago_profiles, sep="\t", dtype=str, low_memory=False
    )
    pair_keys = ["yago_entity", "semopenalex_entity"]
    if dataframe.duplicated(pair_keys).any():
        raise SystemExit("Graph-neighbor score file contains duplicate pairs")
    ...
    dataframe["neighbor_tfidf_score"] = pd.to_numeric(
        dataframe["neighbor_tfidf_score"], errors="coerce"
    ).fillna(0.0)
    ...
    dataframe["combined_score"] = (
        ...
        + args.neighbor_weight * dataframe["neighbor_tfidf_score"]
    )
